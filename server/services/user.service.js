...
exports.get_user_details = async (req) => {
    ...
    const response = await user_repository.findOne({criteria:{
        uuid: user_id
    }})
    response.password = undefined
    return response
...
exports.patch_user_details = async (req) => {
    ...
    const response = await user_model.updateOne({ id: user_id }, { ...update_data })
    response.password = undefined
...
exports.set_user_inactive = async (req) => {
    const { user } = req.body
    const { user_id } = user
    const response = await user_model.findOneAndUpdate({ id: user_id }, { status: "inactive" }, { upsert: false, new: true })
    response.password = undefined
    return response
}
exports.set_user_active = async (req) => {
    const { user } = req.body
    const { user_id } = user
    const response = await user_model.findOneAndUpdate({ id: user_id }, { status: "active" }, { upsert: false, new: true })
    response.password = undefined
    return response
}
...
exports.set_user_image = async (req) => {
    ...
    const resp = await user_model.updateOne({id:user_id} , {image})
    return resp
}
